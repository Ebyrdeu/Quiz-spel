...
const classDB = [
    ...
    namesOnly = classDB.map(e => e.name);
const shuffled = namesOnly.sort(() => 0.5 - Math.random());
...
const personName = () => {
    ...
    let d = 0;
    btn.forEach((e, i) => {
        e.textContent = shuffled[i];
        console.log(`%c${e.textContent} `, 'color: #bada55', shuffled[i] === classDB[randomCorrectName].name);
        
        if (shuffled[i] !== classDB[randomCorrectName].name) {
            d++;
            if (d <= 3) {
              
            } else {
                e.textContent = classDB[randomCorrectName].name
            }
        }
    })
    ...
    console.log(`%c${classDB[randomCorrectName].photo}  ${btn.textContent = classDB[randomCorrectName].name}`, 'color: gold');

    console.log('%c-----------------------------', '  color: red');

    btn.forEach(e => console.log(e.textContent))

};

// Attach Classmate Info into Image
const render = () => {
    randomCorrectName = Math.floor(Math.random() * namesOnly.length)

    header.innerHTML = `<p class="text-success fs-3 fw-bold text-center">Correct: ${correct}/${all} </p>`;

    gameBox.innerHTML = `<section class=""> <img src="${classDB[randomCorrectName].photo}" class="rounded img-fluid  w-25 mb-5 " alt="${classDB[randomCorrectName].name}"></section>`;
    console.clear()
    personName();
};
